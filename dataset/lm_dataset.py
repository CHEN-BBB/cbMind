...
# 设置环境变量，禁用tokenizers库的并行处理，以避免在多线程环境中出现问题
...
def pre_processing_chat(conversations, add_system_ratio=0.2):
    """
    对话前处理：以一定概率随机插入 system 消息。数据增强手段，提升模型对 system prompt 存在与否两种情况的适应能力。

    特点：
    - 只有当首条消息不是 system 角色时才可能插入。
    - add_system_ratio 控制插入概率（默认 20%），引入随机性可提升模型对有/无 system prompt 两种情况的泛化能力。
...
def post_processing_chat(prompt_content, empty_think_ratio=0.05):
    ...
    特点：
    - 针对带 CoT（chain-of-thought）格式的模型，apply_chat_template 有时会渲染出 "<think>\n\n</think>\n\n" 这样的空思考块占位符。
    - 大部分情况下（概率 1 - empty_think_ratio = 95%）直接删除该空块，防止模型学到"无意义思考"的坏习惯。
...
class PretrainDataset(Dataset):
    def __init__(self, data_path, tokenizer, max_length=512):
        ...
        self.max_length = max_length  # 预设最大序列长度，超过部分会被截断
    ...
    def __getitem__(self, index):
        ...
        tokens = self.tokenizer(
            ...
            truncation=True,  # 超过 max_length 的部分会被截断
        ).input_ids  # 直接获取 token ID 列表，不返回其他信息
        ...
        input_ids = torch.tensor(input_ids, dtype=torch.long)  # 转换为 PyTorch 张量，数据类型为 long（整数）
        ...
        attention_mask = (input_ids != self.tokenizer.pad_token_id).long()  # 1 表示非 PAD，0 表示 PAD
...
class SFTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # ！修正：强制将 content 转为字符串，彻底消灭 Undefined 导致的非字符串问题
        fixed_conv = []
        for msg in conversations:
            fixed_conv.append({
                "role": str(msg["role"]),
                "content": str(msg["content"])  # 强制转字符串，彻底消灭 Undefined
            })
        prompt = self.create_chat_prompt(fixed_conv)
...
class RLAIFDataset(Dataset):
    ...
    def create_chat_prompt(self, conversations):
        ...
        prompt = post_processing_chat(prompt)  # 清理可能的空 <think> 块
