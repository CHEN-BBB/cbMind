...
__package__ = "trainer"  # 明确指定当前模块的包名，确保相对导入正常工作
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))  # 将项目根目录添加到系统路径，确保可以导入model和dataset等模块
...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels, attention_mask) in enumerate(
        ...
        # 学习率调度：根据当前的训练进度动态调整学习率，通常采用线性衰减或预热策略
        lr = get_lr(epoch * iters + step, args.epochs * iters, args.learning_rate)

        for param_group in optimizer.param_groups:
            param_group["lr"] = lr  # 更新优化器的学习率

        with autocast_ctx:  # 混合精度上下文，自动选择适当的数值精度进行计算，关键运算使用更高精度以保持数值稳定性
            # 前向传播
            res = model(
        ...
        scaler.scale(loss).backward()  # 反向传播：使用梯度缩放器对损失进行缩放，防止在半精度训练中出现梯度下溢

        if step % args.accumulation_steps == 0:  # 每隔 accumulation_steps 步更新一次参数
            # scaler.unscale_(): 还原梯度的真实值
            scaler.unscale_(optimizer)

            torch.nn.utils.clip_grad_norm_(model.parameters(), args.grad_clip)  # 梯度裁剪：限制梯度的最大范数，防止梯度爆炸
            ...
            optimizer.zero_grad(set_to_none=True)  # 清空梯度，set_to_none=True可以节省内存并可能提高性能
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="cbMind Pretraining")
    ...
    parser.add_argument(
        "--wandb_project", type=str, default="cbMind-Pretrain", help="wandb项目名"
    )
    ...
    if args.use_wandb and is_main_process():
        ...
        # 构建实验名称，包含关键超参数
        wandb_run_name = f"cbMind-Pretrain-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LearningRate-{args.learning_rate}"
        wandb.init(
    ...
    scaler = torch.cuda.amp.GradScaler(enabled=(args.dtype == "float16"))  # 仅在使用float16时启用梯度缩放
