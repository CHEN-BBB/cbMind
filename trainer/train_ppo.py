...
# 将上级目录加入模块搜索路径，以便导入model和dataset等模块
...
class CriticModel(cbMindForCausalLM):
    ...
    def forward(self, input_ids=None, attention_mask=None, **kwargs):
        ...
        # outputs[0]是最后一层的隐藏状态，形状为[B, seq_len, hidden_size]
        hidden_states = self.model.norm(outputs[0])
        # 通过线性层输出每个位置的value，形状为[B, seq_len, 1]，然后squeeze掉最后一维得到[B, seq_len]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="cbMind PPO (Proximal Policy Optimization)"
    )
    ...
    parser.add_argument(
        "--wandb_project", type=str, default="cbMind-PPO", help="wandb项目名"
    )
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb_run_name = f"cbMind-PPO-Epoch-{args.epochs}-BS-{args.batch_size}-LR-{args.learning_rate}"
        wandb.init(
