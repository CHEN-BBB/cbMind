"""
cbMind LoRA (Low-Rank Adaptation) 微调脚本  # ！修正：原MiniMind改为cbMind
...
# cbMind相关组件
from model.model import cbMindConfig  # 模型配置
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="cbMind LoRA Fine-tuning")
    ...
    parser.add_argument(
        "--wandb_project", type=str, default="cbMind-LoRA", help="wandb项目名"
    )
    ...
    # 📚 模型配置初始化
    # cbMindConfig: 定义模型的超参数，如隐藏维度、层数、是否使用MoE
    ...
    if args.use_wandb and is_main_process():
        ...
        # 📚 实验名称生成
        # 包含关键参数，便于识别不同的实验配置
        wandb_run_name = f"cbMind-LoRA-{args.lora_name}-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LR-{args.learning_rate}"
        wandb.init(
